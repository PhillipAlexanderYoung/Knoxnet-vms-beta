"""First-run startup wizard with automatic camera discovery.

Two-page flow:
  Page 1 -- Welcome + auto network scan with inline-editable 11-column table
             (encoder auto-detection, IP grouping, bulk operations)
  Page 2 -- All Set summary
"""
...
from PySide6.QtWidgets import (
    ...
    QStackedWidget, QWidget, QCheckBox, QTableWidget, QTableWidgetItem,
    QHeaderView, QProgressBar, QFrame, QAbstractItemView,
)
...
from desktop.utils.network_scanner import (
    ...
    test_rtsp_connection, VENDOR_RTSP_TEMPLATES,
)

logger = logging.getLogger("StartupWizard")
...
QLineEdit {{
    ...
    border: 1px solid {_BORDER}; border-radius: 4px; padding: 8px; font-size: 14px;
}}
QLineEdit:focus {{ border: 1px solid {_ACCENT}; }}
QCheckBox {{ color: {_TEXT_MAIN}; spacing: 8px; }}
...
QPushButton {{
    ...
    border: 1px solid {_BORDER}; border-radius: 6px; padding: 10px 28px; font-size: 14px;
}}
...
class CameraDiscoveryWizard(QDialog):
    """Two-page first-run wizard with automatic camera discovery and inline editing."""

    def __init__(self, prefs_file: Path, cameras_json_path: Path, parent=None):
        ...
        self.setWindowTitle("Knoxnet VMS Beta Setup")
    ...
    def _build_page_scan(self):
        ...
        title = QLabel("Welcome to Knoxnet VMS Beta")
        title.setFont(QFont("Segoe UI", 20, QFont.Weight.Bold))
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lay.addWidget(title)

        subtitle = QLabel(
            "Scanning your network for IP cameras and encoders. "
        ...
        self.scan_status = QLabel("Initializing scan\u2026")
        ...
        bulk_lay.setContentsMargins(10, 5, 10, 5)
        ...
        self.bulk_user.setFixedWidth(80)
        ...
        self.bulk_pass.setFixedWidth(80)
        ...
        self.bulk_port.setFixedWidth(50)
        ...
        for lbl_text, attr, placeholder, width in [
            ...
            ("RTSP", "manual_url", "rtsp://user:pass@ip:554/stream", 300),
        ]:
            lbl = QLabel(lbl_text)
        ...
        self.gpu_badge.setFont(QFont("Segoe UI", 10))
        self.gpu_badge.setStyleSheet(
            f"color: {_TEXT_DIM}; background: {_BG_LIGHT}; "
            f"border: 1px solid {_BORDER}; border-radius: 4px; padding: 4px 10px;"
        )
        lay.addWidget(self.gpu_badge, alignment=Qt.AlignmentFlag.AlignLeft)
        ...
        self.btn_add_selected = QPushButton("Add Selected && Continue")
    ...
    def _build_page_done(self):
        ...
        self.done_summary.setFont(QFont("Segoe UI", 13))
        self.done_summary.setStyleSheet(f"color: {_TEXT_DIM};")
        ...
        tray_hint = QLabel(
            "Knoxnet VMS Beta runs in your system tray.\n"
            "Right-click the tray icon to manage cameras,\n"
            "layouts, and detection settings."
        )
        tray_hint.setFont(QFont("Segoe UI", 11))
        tray_hint.setStyleSheet(f"color: {_TEXT_DIM};")
        tray_hint.setAlignment(Qt.AlignmentFlag.AlignCenter)
        tray_hint.setWordWrap(True)
        lay.addWidget(tray_hint)

        lay.addStretch()

        btn_row = QHBoxLayout()
        btn_row.addStretch()
        btn_finish = QPushButton("Launch Knoxnet VMS Beta")
        btn_finish.setStyleSheet(_BTN_PRIMARY)
    ...
    def _build_rtsp_url_from_row(self, row: int) -> str:
        ...
        vendor = mfg_item.text().replace(" Encoder", "") if mfg_item else ""
    ...
    def _on_scan_finished(self, _results: list):
        ...
        if count == 0:
            self.scan_status.setText("Scan complete \u2014 no cameras found. Try adding one manually below.")
        else:
            enc_msg = ""
            if encoder_ips:
    ...
    def _update_add_button(self, _=None):
        ...
        if remaining <= 0:
            ...
            self.btn_add_selected.setText(f"Add {remaining} of {checked} && Continue")
            self.limit_banner.setText(
            ...
            if checked > 0:
                self.btn_add_selected.setEnabled(True)
                self.btn_add_selected.setText(f"Add {checked} Camera{'s' if checked != 1 else ''} && Continue")
            else:
                self.btn_add_selected.setEnabled(False)
                self.btn_add_selected.setText("Add Selected && Continue")
            self.limit_banner.setVisible(False)
            self.beta_limit_note.setVisible(False)
    ...
    def _update_gpu_badge(self):
        try:
            ...
            if caps.has_gpu:
                self.gpu_badge.setText("\u26a1 GPU: NVIDIA CUDA detected \u2014 hardware acceleration available")
                self.gpu_badge.setStyleSheet(
                    f"color: {_GREEN}; background: {_BG_LIGHT}; "
                    f"border: 1px solid {_BORDER}; border-radius: 4px; padding: 4px 10px;"
                )
            else:
                self.gpu_badge.setText("\u25cf CPU mode \u2014 MobileNetSSD object detection ready")
        except Exception:
            self.gpu_badge.setText("\u25cf Object detection: Ready")
    ...
    def _on_add_selected(self):
        ...
        self._added_count += self._save_cameras(entries)
        self._go_to_done()
    ...
    def _on_add_manual(self):
        ...
        if saved:
            ...
            self.manual_error.setText(f"\u2713 \"{name}\" added successfully.")
            self._update_add_button()
    ...
    def _update_done_page(self):
        if self._added_count > 0:
            cam_word = "camera" if self._added_count == 1 else "cameras"
            self.done_summary.setText(
                f"{self._added_count} {cam_word} added, object detection ready.\n\n"
                "Your cameras will appear in the tray menu momentarily."
            )
        else:
            self.done_summary.setText(
                "No cameras were added.\n\n"
                "You can add cameras anytime from the tray menu."
            )
    ...
    def _save_cameras(self, entries: list) -> int:
        ...
        try:
            ...
            for e in entries:
                logger.info("Wizard: saved camera '%s' (id=%s)", e.get("name"), e.get("id"))
            return len(entries)
        except Exception as exc:
            logger.error("Wizard: failed to save cameras: %s", exc)
            return 0
    ...
    def _mark_setup_complete(self):
        ...
        try:
            ...
            logger.info("Wizard: marked setup_complete")
        except Exception as exc:
            logger.error("Wizard: failed to write prefs: %s", exc)
...
StartupWizard = CameraDiscoveryWizard
